 ...
 * back at the seeker.
 */
public class EntitySeeker extends EntityGhast implements IFullMoonMob {
    ...
    protected void updateEntityGoal() {
        ...
        if (distanceSq < 4096.0) {
            ...
            if (this.attackCounter == 20) {
                if (!this.worldObj.isRemote) {
                    this.shootFireballAtEntity(this.targetedEntity, this.canEntityBeSeen(this.targetedEntity));
                }
     ...
     * @param canSee True if the target can be seen. */
    public void shootFireballAtEntity(Entity target, boolean canSee) {
        double x = target.posX - this.posX;
        double y = target.boundingBox.minY - (this.posY + this.height / 2.0F);
        double z = target.posZ - this.posZ;
        this.worldObj.playAuxSFXAtEntity((EntityPlayer)null, 1008, (int)this.posX, (int)this.posY, (int)this.posZ, 0);
        EntitySeekerFireball fireball = new EntitySeekerFireball(this.worldObj, this, x, y, z, canSee);
